...
class FixedEdgesToGlobalsAggregator(_base.AbstractModule):
  """Aggregates all edges into globals."""

  def __init__(self, reducer, name="edges_to_globals_aggregator"):
    """Initializes the FixedEdgesToGlobalsAggregator module.


    """
    super(FixedEdgesToGlobalsAggregator, self).__init__(name=name)
    self._reducer = reducer
  ...
  def _build(self, graph):
    ...
    return self._reducer( tf.reshape(graph.edges, (graph.globals.shape[0],int(graph.edges.shape[0]/graph.globals.shape[0])*graph.edges.shape[1])) )


class FixedNodesToGlobalsAggregator(_base.AbstractModule):
  """Aggregates all nodes into globals."""

  def __init__(self, reducer, name="nodes_to_globals_aggregator"):
    """Initializes the FixedNodesToGlobalsAggregator module.

    """
    super(FixedNodesToGlobalsAggregator, self).__init__(name=name)
    self._reducer = reducer
  ...
  def _build(self, graph):
    ...
    graph_index = tf.range(num_graphs)
    indices = utils_tf.repeat(graph_index, graph.n_node, axis=0)
    return self._reducer(tf.reshape(graph.nodes, (graph.globals.shape[0],int(graph.nodes.shape[0]/graph.globals.shape[0])*graph.nodes.shape[1])) )
...
class FixedGlobalBlock(_base.AbstractModule):
  """Global block for fixed Graphs.
  ...
  def __init__(self,
    ...
    Args:
      ...
      use_globals: (bool, default=True) Whether to condition on global
        attributes.
      nodes_reducer: Reduction to be used when aggregating nodes.
      edges_reducer: Reduction to be used when aggregating edges.
      name: The module name.
    ...
    super(FixedGlobalBlock, self).__init__(name=name)
    ...
    with self._enter_variable_scope():
      ...
      if self._use_edges:
        if edges_reducer is None:
          ...
          self._edges_aggregator = FixedEdgesToGlobalsAggregator(edges_reducer)
      if self._use_nodes:
        if nodes_reducer is None:
          self._use_nodes = False
        else:
          self._nodes_aggregator = FixedNodesToGlobalsAggregator(nodes_reducer)
