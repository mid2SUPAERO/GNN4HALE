...
def _make_custom_global_block_opt(global_block_opt, edgetoglobalreducer,nodetoglobalreducer):
  """Default options to be used in the GlobalBlock of a generic GraphNetwork."""
  global_block_opt = dict(global_block_opt.items()) if global_block_opt else {}
  for k, v in _DEFAULT_GLOBAL_BLOCK_OPT.items():
    global_block_opt[k] = global_block_opt.get(k, v)

  """Customized reducer part"""
  '''
  if edgetoglobalreducer is None:
      global_block_opt["use_edges"] = False

  if nodetoglobalreducer is None:
      global_block_opt["use_nodes"] = False
  '''

  global_block_opt["edges_reducer"] = global_block_opt.get("edges_reducer", edgetoglobalreducer)
  global_block_opt["nodes_reducer"] = global_block_opt.get("nodes_reducer", nodetoglobalreducer)


  return global_block_opt


class FixedGraphNetwork(_base.AbstractModule):
  ...
  def __init__(self,
    ...
    Args:
      ...
      global_model_fn: A callable that will be passed to GlobalBlock to perform
        per-global computations. The callable must return a Sonnet module (or
        equivalent; see GlobalBlock for details).
      edgetoglobalreducer: TO BE DETAILED.
      nodetoglobalreducer: TO BE DETAILED .
      ...
      name: The module name.
    """
    super(FixedGraphNetwork, self).__init__(name=name)
    ...
    global_block_opt = _make_custom_global_block_opt(global_block_opt, edgetoglobalreducer,nodetoglobalreducer)
    ...
    with self._enter_variable_scope():
      ...
      self._global_block = blocks.FixedGlobalBlock(
          global_model_fn=global_model_fn,
          **global_block_opt)
...
class FixedGEncoderGraphNetwork(_base.AbstractModule):
  ...
  def __init__(self,
    ...
    Args:
      ...
      name: The module name.
    """
    super(FixedGEncoderGraphNetwork, self).__init__(name=name)
    global_block_opt = _make_custom_global_block_opt(global_block_opt, edgetoglobalreducer,nodetoglobalreducer)
  ...
  def _build(self, graph):
    ...
    return self._global_block(graph)
