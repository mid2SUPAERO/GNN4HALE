#generation of sudden force , symmetric scenario to be used as a dummy, well mastered control surface deflection
import sys
...
def generate_dyn_file():
    ...
    if with_dynamic_forces:
        f1 = impulse_intensity_input
        dynamic_forces = np.zeros((n_node, 6))
        #app_node = [int(num_node_main - 1), int(num_node_main)]
        app_node = impulse_nodes[key]
       



        for i in range(len(app_node)):
            #dynamic_forces[app_node[i], 2] = f1   #force in  Z
            if i<10:
                dynamic_forces[app_node[i], 1] = -f1*0.37   #force in  Y
                dynamic_forces[app_node[i], 2] = f1*0.93   #force in  Z
            elif i<20:
                dynamic_forces[app_node[i], 1] = f1*0.37   #force in  Y
                dynamic_forces[app_node[i], 2] = f1*0.93   #force in  Z
            else:
                dynamic_forces[app_node[i], 2] = f1  #force in  Z
            #dynamic_forces[app_node[i], 4] = f1   #moment in  Y
        

        force_time = np.zeros((n_tstep,))
        limit = round(0.05 / dt)
        force_time[itstart_deflection:itend_deflection ] = 1
...
impulse_intensity = [100]
impulse_duration = [0.5,1.0]
impulse_nodes=  {'ail':[7,8,15,16],'elev':[27,28,31,32]} #,

case_name_root='HALE_SYMM_test_F'
route = '/home/dmsm/m.colombo/Documents/sharpy/data/'+case_name_root+'/'
os.makedirs(route,exist_ok=True)

for tdeflection in impulse_duration :
    for impulse_intensity_input in impulse_intensity :
        for key in impulse_nodes:
            case_name = case_name_root+'_INT'+str(impulse_intensity_input)+'_T'+str(tdeflection)+'_'+key
            ...
            eiy = 2e4*sigma_main
            eiz = 4e6*sigma_main
            m_bar_main = sigma_weight*0.75
            j_bar_main = sigma_weight*0.075

            length_fuselage = 10
            offset_fuselage = 0
            sigma_fuselage = 10
            m_bar_fuselage = sigma_weight*0.2
            j_bar_fuselage = sigma_weight*0.08
            ...
            ea_fin = 0.5
            sigma_tail = 10
            m_bar_tail = sigma_weight*0.3
            j_bar_tail = sigma_weight*0.08
            ...
            itstart_deflection=int(2/tstep_factor) 
            itend_deflection= itstart_deflection+int(tdeflection/dt)
