...
gust_intensity_range = np.arange(0.3,0.6,0.3)
gust_length_range =  np.arange(0.9,1.2,0.3)
case_name_root='HALE_50_2el_light_stiff'
route = '/home/dmsm/m.colombo/Documents/sharpy/data/'+case_name_root+'/'
os.makedirs(route,exist_ok=True)

for gust_intensity_input in gust_intensity_range :
    for gust_length_input in gust_length_range :

        
        case_name = case_name_root+'_'+'i'+np.array2string(gust_intensity_input, precision=2)+'l'+np.array2string(gust_length_input, precision=2)
        ...
        # gust settings
        gust_intensity = gust_intensity_input  # 0.2
        gust_length = gust_length_input * u_inf
